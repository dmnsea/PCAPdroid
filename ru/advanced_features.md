...
- Если вы используете фильтр приложений, вы можете упустить DNS трафик генерируемый системным сервисом Android `netd`, который выполняет разрешение имен от имени приложений.
...
При записи дампа в [формат Pcapng](paid_features#53-формат-pcapng), метаданные из PCAPdroid добавляются как [кастомные блоки Pcapng](https://www.ietf.org/archive/id/draft-tuexen-opsawg-pcapng-04.html#name-custom-block-2) с помощью [62652 PEN](https://www.iana.org/assignments/enterprise-numbers/?q=62652). В частности:
