...
**Note**: be sure to use the [Pcapng format](paid_features#53-pcapng-format) if you create packet captures in root mode. PCAPdroid only stores the packet direction in the Pcapng format; if you use PCAP, loading a previous root capture may show the source and destination hosts of the connections swapped
...
**Note**: the PCAPdroid extensions are always enabled when the Pcapng format is used
